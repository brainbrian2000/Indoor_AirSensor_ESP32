#line 1 "E:\\.NTU\\IPCS\\airsensor\\test_code\\Wifi_connect.cpp"
#include <Wifi.h>
#include <esp_wifi.h>
#include <esp_err.h>
// #include <esp_wpa.h>
    ...
    public:
        ...
        bool connect_peap(){
            ...
            if(WiFi.status() == WL_CONNECTED){
                ...
                Serial.println(WiFi.config(WiFi.localIP(),IPAddress(192,168,1,1),IPAddress(255,255,255,0),IPAddress(8,8,8,8),IPAddress(8,8,8,8)));
            }else{
                return_status = false;
        ...
        bool PingTest(){
            ...
            for(int i = 0; i < 5; i++){
                bool success = Ping.ping(IPAddress(ip[i][0], ip[i][1], ip[i][2], ip[i][3]), 3);
                if(!success){
